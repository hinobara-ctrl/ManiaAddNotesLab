...
- `nextBehavioralPhase` identifica por separado la siguiente fase que cambiaría conducta; no puede ser la misma que `nextRecommendedPhase`, debe conservar autorización explícita y no se vuelve accionable por completar investigación previa.
...
`README.md` y `PROJECT_STATUS.md` deben contener exactamente un bloque `PROJECT-STATE`. El bloque proyecta, como campos separados, la fase actual, la siguiente candidata accionable —o `none`— y su autorización —o `N/A`—, la siguiente fase conductual y su autorización, el prerequisite bloqueado, la decisión de rama, la policy conductual, el cambio conductual y el snapshot de tests. El orden de los campos no es semántico. `ROADMAP.md` debe contener filas exactas e independientes para current, next actionable cuando exista, next behavioral y blocker; un identificador prefijo como `F2` nunca puede satisfacer por accidente una fila `F2.1` o `F2.ACQ`.
