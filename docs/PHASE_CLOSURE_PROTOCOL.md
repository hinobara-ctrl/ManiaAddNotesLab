...
- `nextRecommendedPhase` identifica una fase de investigación accionable recomendada que todavía no comenzó; cuando existe, debe ser distinta de `currentPhase`, tener estado `NEXT`, no apuntar a un prerequisite `BLOCKED` y declarar `authorization` explícita. Puede ser `null` cuando un cierre no recomienda investigación adicional; en ese caso no debe existir ninguna fase `NEXT` y los bloques maestros muestran `none` / `N/A`.
- `nextBehavioralPhase` identifica por separado la siguiente fase que cambiaría conducta; cuando existe, no puede ser la misma que `nextRecommendedPhase`, debe conservar autorización explícita y no se vuelve accionable por completar investigación previa. Puede ser `null` tras un cierre que no autoriza sucesora; los bloques muestran `none` / `N/A`.
- `phaseContracts` fija contratos machine-readable para fases cuya clase no puede inferirse sólo del status. `ResearchShadow` exige `behaviorChange=false`; `BehaviorChanging`, `behaviorChange=true`; `BlockedPrerequisite`, status `BLOCKED`; y `Deferred`, status `DEFERRED`. Los marcadores mínimos del roadmap técnico deben coincidir con esos contratos.
...
- `behaviorChange` refleja el contrato de la fase cerrada actual, no la policy promovida por defecto. Una fase experimental conductual usa `true` aunque termine en rollback y sin promoción; una actualización puramente documental/research usa `false` y se proyecta como `Behavior change: none`.
...
`README.md` y `PROJECT_STATUS.md` deben contener exactamente un bloque `PROJECT-STATE`. El bloque proyecta, como campos separados, la fase actual, la siguiente candidata accionable —o `none`— y su autorización —o `N/A`—, la siguiente fase conductual —o `none`— y su autorización —o `N/A`—, el prerequisite bloqueado, la decisión de rama, la policy conductual, el cambio conductual y el snapshot de tests. El orden de los campos no es semántico. `ROADMAP.md` debe contener filas exactas e independientes para current, next actionable/behavioral cuando existan y blocker; un identificador prefijo como `F2` nunca puede satisfacer por accidente una fila `F2.1` o `F2.ACQ`.
