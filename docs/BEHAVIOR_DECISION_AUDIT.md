...
| Exact interior relation membership | G1.DESIGN research shadow | decide si una propuesta legacy exacta pertenece al result set original chart-local/full-chart de su query G1.0 | No | IMPLEMENTATION_ONLY | G1.DESIGN | No activa | G1.DESIGN COMPLETE/READY | Zero RNG; alternatives conservan membership; same-parent sólo se publica como provenance; G1.GATE/G1 siguen NOT_AUTHORIZED |
...
El inventario contiene **104 decisiones**: 3 `USER_INTENT`, 6 `HARD_VALIDITY_INVARIANT`, 9 `CURRENT_POLICY_CONTRACT`, 1 `FUTURE_POLICY_CONTRACT`, 49 `MAPPER_DERIVED_CANDIDATE`, 25 `IMPLEMENTATION_ONLY` y 11 `OPEN_DESIGN_DECISION`. D1 y D1.SAFETY cerraron Outcome C/PARKED; SAFETY.PROV y G1.0 cerraron Outcome A shadow-only. G1.DESIGN cerró READY al demostrar membership exacta sin convertir counts en authority. `G1.GATE` es sólo un candidato **NOT_AUTHORIZED**. Ninguna policy mapper-derived gobierna el selector normal; el default continúa legacy.
