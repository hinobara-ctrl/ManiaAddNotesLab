...
El inventario contiene **101 decisiones**: 3 `USER_INTENT`, 6 `HARD_VALIDITY_INVARIANT`, 9 `CURRENT_POLICY_CONTRACT`, 1 `FUTURE_POLICY_CONTRACT`, 49 `MAPPER_DERIVED_CANDIDATE`, 22 `IMPLEMENTATION_ONLY` y 11 `OPEN_DESIGN_DECISION`. D1 y D1.SAFETY cerraron Outcome C/PARKED. La regla general añadida exige que un safety failure conductual sea causalmente atribuible a su branch o quede `Unattributable`; condiciones de source/control no se imputan silenciosamente a treatment. No queda una fase accionable. Ninguna policy mapper-derived gobierna el selector normal; el default continúa legacy.
