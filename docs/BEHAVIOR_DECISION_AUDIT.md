...
El inventario contiene **104 decisiones**: 3 `USER_INTENT`, 6 `HARD_VALIDITY_INVARIANT`, 9 `CURRENT_POLICY_CONTRACT`, 1 `FUTURE_POLICY_CONTRACT`, 49 `MAPPER_DERIVED_CANDIDATE`, 25 `IMPLEMENTATION_ONLY` y 11 `OPEN_DESIGN_DECISION`. G1.DESIGN está READY/RECERTIFIED: membership del universo del builder, no post-geometry ni utility. `G1.GATE` es sólo candidato **NOT_AUTHORIZED**. Ninguna policy mapper-derived gobierna el selector normal; el default continúa legacy.
