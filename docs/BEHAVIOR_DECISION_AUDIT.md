...
| ChordCompletion resulting-state policy | D1 behavioral A/B | futura validación del estado acumulado antes de aceptar una composición | Sí | FUTURE_POLICY_CONTRACT | D1 histórico | Sí, sólo versionado | D1 FUTURE | `behaviorChange=true`; requiere prerequisites y autorización separada; D1.0 no la activa |
...
El inventario contiene **100 decisiones**: 3 `USER_INTENT`, 6 `HARD_VALIDITY_INVARIANT`, 9 `CURRENT_POLICY_CONTRACT`, 1 `FUTURE_POLICY_CONTRACT`, 49 `MAPPER_DERIVED_CANDIDATE`, 21 `IMPLEMENTATION_ONLY` y 11 `OPEN_DESIGN_DECISION`. El contrato D1 es futuro y no está activo. C1.2 representa la señal exact-head; D0–D0.2 completion/context; F1 evidence state; F2 gaps/backoff; F2.1 information loss; F2.2 compatibility condicional; F2.3 domain/truth/acquisition boundaries; E recurrence/boundaries inferidas; E.1 failure diagnostics. D1.0 cerró Outcome A como prerequisite research sin conducta; D1.GATE cerró READY con preregistración verificable, sin A/B; no queda candidata research accionable y D1 sigue como candidato conductual `FUTURE/NOT_AUTHORIZED`. Ninguna fase gobierna el selector; la sustitución conductual continúa en HOLD.
