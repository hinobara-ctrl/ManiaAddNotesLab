...
| Forward quantization compatibility set | F2.2 research shadow | serializa domains latentes explícitos y conserva empty/singleton/multiple bajo assumptions visibles | No mientras permanezca shadow | IMPLEMENTATION_ONLY | F2.2 | No activa | F2.2 COMPLETE/B | Formalizable sólo condicionalmente; domain mapper-derived y labels humanos ausentes; sin winner ni generation |
| Quantization domain/truth catalog | F2.3 research shadow | canoniza domain content y representa truth source→destination con source/circularity/independence explícitos | No mientras permanezca shadow | IMPLEMENTATION_ONLY | F2.3 | No activa | F2.3 COMPLETE/B | CONTINUE CONDITIONALLY sólo tras package pre-export independiente; no F2.4 ni quantizer |
| Adaptive recurrence/boundary representation | Phase E research shadow | recurrence exacta no contigua y stable-run boundaries inferidas con method/parameter identity | No mientras permanezca shadow | IMPLEMENTATION_ONLY | E | No activa | E COMPLETE/B | Recurrence y segmentation quedan B; sin Section, selector, fuzzy fallback ni generation |
| Exact recurrence failure diagnostics | E.1 research shadow | estratifica exact absence, donor multiplicity, observables y refinements contrafactuales sin cambiar identity | No mientras permanezca shadow | IMPLEMENTATION_ONLY | E.1 | No activa | E.1 COMPLETE/B | Rama recurrence PARKED; spacing/held/type no reciben authority ni entran al resolver |
| Resulting-state composition feasibility | D1.0 research shadow | audita CompletionSets conjuntos frente a members marginales sin seleccionar ni acumular candidatos | No; cierre Outcome A shadow-only | IMPLEMENTATION_ONLY | D1.0 | No activa | D1.0 COMPLETE/A | `behaviorChange=false`; un joint witness requiere la misma occurrence original; 2.913 targets marginal-only |
| Resulting-state behavioral experiment gate | D1.GATE research shadow | contrato congelado `k=1→2`, `ReducedOnly`, joint-admit, abstain-without-reroll, métricas, stopping y rollback legacy | No; cierre READY shadow-only | IMPLEMENTATION_ONLY | D1.GATE | No activa | D1.GATE COMPLETE/READY | No ejecuta A/B ni autoriza D1; hash D574631B… |
...
| Exact interior relation membership | G1.DESIGN research shadow | decide si una propuesta legacy exacta pertenece al result set original chart-local/full-chart de su query G1.0 | No | IMPLEMENTATION_ONLY | G1.DESIGN | No activa | G1.DESIGN COMPLETE/READY — RECERTIFIED | Shapes pre-geometry, no efectos; RNG-free estructural; construction provenance descriptiva; G1.GATE/G1 NOT_AUTHORIZED |
| Exact interior relation runtime admission | G1.GATE treatment research-only | tras shape/lane/geometry, commitea la misma propuesta si membership es Unique/AmongAlternatives y suprime en los demás estados | Sí, sólo treatment explícito | FUTURE_POLICY_CONTRACT | G1.GATE | Sí, sólo versionado | G1.GATE COMPLETE/NEEDS_REVIEW — NO PROMOTION | 117 ADMIT y 889 ABSTAIN; 0 RNG/reroll/replacement; 9 hard violations atribuibles activan stop |
| Canonical playable geometry authority | SAFETY.REMEDIATION.GATE treatment research-only | deriva beats de collision exclusivamente de integer milliseconds; latent queda para intent/evidence/G1 | Sí, sólo treatment explícito | FUTURE_POLICY_CONTRACT | SAFETY.REMEDIATION.GATE | Sí, sólo versionado | COMPLETE/VALIDATION HARDENING NEEDS_REVIEW — NO PROMOTION | 224 pares; 215→0 condiciones hard; causalidad fuerte 188 directos + 22 downstream + 5 unresolved; 0 RNG, drift G1 o fallos de reparse; default legacy |
...
El inventario contiene **106 decisiones**: 3 `USER_INTENT`, 6 `HARD_VALIDITY_INVARIANT`, 9 `CURRENT_POLICY_CONTRACT`, 3 `FUTURE_POLICY_CONTRACT`, 49 `MAPPER_DERIVED_CANDIDATE`, 25 `IMPLEMENTATION_ONLY` y 11 `OPEN_DESIGN_DECISION`. SAFETY.REMEDIATION.GATE conserva evidencia de 0 violaciones treatment, pero su causalidad individual endurecida queda `NEEDS_REVIEW`; ninguna policy mapper-derived gobierna el selector normal y el default continúa legacy.
...
## SAFETY.REMEDIATION.GATE — decisión experimental certificada

Estado: **COMPLETE — VALIDATION HARDENING NEEDS_REVIEW / NO PROMOTION**. El overload research integra `safety-remediation-canonical-playable.1`: placement consulta beats reconstruidos desde integer milliseconds, mientras latent values conservan intención, provenance e identidad G1. Es un cambio conductual únicamente cuando el runner suministra la configuración congelada; normal CLI/Web sigue en `legacy-experimental.1`.

En 224 pares, control produjo 215 condiciones HardValidity y treatment cero. Los 209 casos conocidos y 6 adicionales se procesaron individualmente: 188 rechazos directos, 22 propuestas causalmente inalcanzables y 5 `UNRESOLVED` bajo la definición fuerte. El gate consumió 0 RNG y no cambió HardValidity, serializer, candidate construction ni G1 evidence. El resultado `NEEDS_REVIEW` no concede promoción, default ni autoridad a una sucesora.
