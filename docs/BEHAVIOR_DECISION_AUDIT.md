...
| Resulting-state composition feasibility | D1.0 research shadow | auditará CompletionSets conjuntos frente a members marginales sin seleccionar ni acumular candidatos | No mientras permanezca shadow | IMPLEMENTATION_ONLY | D1.0 | No activa | D1.0 NEXT | Prerequisite research; `behaviorChange=false`; un joint witness requiere la misma occurrence original |
...
El inventario contiene **99 decisiones**: 3 `USER_INTENT`, 6 `HARD_VALIDITY_INVARIANT`, 9 `CURRENT_POLICY_CONTRACT`, 1 `FUTURE_POLICY_CONTRACT`, 49 `MAPPER_DERIVED_CANDIDATE`, 20 `IMPLEMENTATION_ONLY` y 11 `OPEN_DESIGN_DECISION`. El contrato D1 es futuro y no está activo. C1.2 representa la señal exact-head; D0–D0.2 completion/context; F1 evidence state; F2 gaps/backoff; F2.1 information loss; F2.2 compatibility condicional; F2.3 domain/truth/acquisition boundaries; E recurrence/boundaries inferidas; E.1 failure diagnostics. E.1 cierra Outcome B y aparca recurrence refinement. D1.0 queda separado como prerequisite research sin conducta; D1 sigue conductual y no autorizado. Ninguna fase gobierna el selector; la sustitución conductual continúa en HOLD.
