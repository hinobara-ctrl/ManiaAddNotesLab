...
| Resulting-state behavioral experiment gate | D1.GATE research shadow | sólo pre-registraría admisión, abstención, métricas, stopping criteria y rollback | No mientras permanezca shadow | OPEN_DESIGN_DECISION | D1.GATE | No activa | D1.GATE NEXT | No ejecuta A/B ni autoriza D1 |
...
El inventario contiene **100 decisiones**: 3 `USER_INTENT`, 6 `HARD_VALIDITY_INVARIANT`, 9 `CURRENT_POLICY_CONTRACT`, 1 `FUTURE_POLICY_CONTRACT`, 49 `MAPPER_DERIVED_CANDIDATE`, 20 `IMPLEMENTATION_ONLY` y 12 `OPEN_DESIGN_DECISION`. El contrato D1 es futuro y no está activo. C1.2 representa la señal exact-head; D0–D0.2 completion/context; F1 evidence state; F2 gaps/backoff; F2.1 information loss; F2.2 compatibility condicional; F2.3 domain/truth/acquisition boundaries; E recurrence/boundaries inferidas; E.1 failure diagnostics. D1.0 cierra Outcome A como prerequisite research sin conducta; D1.GATE queda NEXT/NOT_AUTHORIZED y D1 sigue conductual/futuro. Ninguna fase gobierna el selector; la sustitución conductual continúa en HOLD.
