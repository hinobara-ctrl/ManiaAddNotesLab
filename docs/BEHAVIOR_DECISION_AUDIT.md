...
El inventario contiene **106 decisiones**: 3 `USER_INTENT`, 6 `HARD_VALIDITY_INVARIANT`, 9 `CURRENT_POLICY_CONTRACT`, 3 `FUTURE_POLICY_CONTRACT`, 49 `MAPPER_DERIVED_CANDIDATE`, 25 `IMPLEMENTATION_ONLY` y 11 `OPEN_DESIGN_DECISION`. SAFETY.REMEDIATION.GATE certificó la autoridad canónica sólo como treatment; ninguna policy mapper-derived gobierna el selector normal y el default continúa legacy.
...
Estado: **COMPLETE — REMEDIATION_RUNTIME_CERTIFIED / NO PROMOTION**. El overload research integra `safety-remediation-canonical-playable.1`: placement consulta beats reconstruidos desde integer milliseconds, mientras latent values conservan intención, provenance e identidad G1. Es un cambio conductual únicamente cuando el runner suministra la configuración congelada; normal CLI/Web sigue en `legacy-experimental.1`.

En 224 pares, control produjo 215 condiciones HardValidity y treatment cero. Los 209 casos conocidos y 6 adicionales quedaron explicados como 188 rechazos directos y 27 propuestas causalmente inalcanzables tras una divergencia anterior; inesperados = 0. El gate consumió 0 RNG y no cambió HardValidity, serializer, candidate construction ni G1 evidence. La decisión certificada no concede promoción, default ni autoridad a una sucesora.
