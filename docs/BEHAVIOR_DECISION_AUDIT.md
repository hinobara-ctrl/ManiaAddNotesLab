...
El inventario contiene **102 decisiones**: 3 `USER_INTENT`, 6 `HARD_VALIDITY_INVARIANT`, 9 `CURRENT_POLICY_CONTRACT`, 1 `FUTURE_POLICY_CONTRACT`, 49 `MAPPER_DERIVED_CANDIDATE`, 23 `IMPLEMENTATION_ONLY` y 11 `OPEN_DESIGN_DECISION`. D1 y D1.SAFETY cerraron Outcome C/PARKED; SAFETY.PROV cerró Outcome A shadow-only. La regla general exige que un safety claim conductual tenga mutation-level provenance capturada durante execution o quede `Unattributable`: final snapshots no atribuyen causalidad, y temporally after divergence != causally downstream. No queda una fase accionable; **ROADMAP REVIEW REQUIRED**. Ninguna policy mapper-derived gobierna el selector normal; el default continúa legacy.
