...
El inventario contiene **97 decisiones**: 3 `USER_INTENT`, 6 `HARD_VALIDITY_INVARIANT`, 9 `CURRENT_POLICY_CONTRACT`, 49 `MAPPER_DERIVED_CANDIDATE`, 19 `IMPLEMENTATION_ONLY` y 11 `OPEN_DESIGN_DECISION`. No hay `FUTURE_POLICY_CONTRACT` activo todavía. C1.2 representa la señal exact-head; D0–D0.2 completion/context; F1 evidence state; F2 gaps/backoff; F2.1 information loss; F2.2 compatibility condicional; F2.3 domain/truth/acquisition boundaries; E recurrence/boundaries inferidas; E.1 failure diagnostics. E.1 cierra Outcome B, aparca recurrence refinement y no cambia conducta. Ninguna fase gobierna el selector; la sustitución conductual continúa en HOLD.
