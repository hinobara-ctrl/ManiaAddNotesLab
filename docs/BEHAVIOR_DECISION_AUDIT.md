...
El inventario contiene **103 decisiones**: 3 `USER_INTENT`, 6 `HARD_VALIDITY_INVARIANT`, 9 `CURRENT_POLICY_CONTRACT`, 1 `FUTURE_POLICY_CONTRACT`, 49 `MAPPER_DERIVED_CANDIDATE`, 24 `IMPLEMENTATION_ONLY` y 11 `OPEN_DESIGN_DECISION`. D1 y D1.SAFETY cerraron Outcome C/PARKED; SAFETY.PROV y G1.0 cerraron Outcome A shadow-only. G1.0 permite representar relaciones interiores completas, pero no convierte counts en authority. `G1.DESIGN` es sólo un candidato **NOT_AUTHORIZED**. Ninguna policy mapper-derived gobierna el selector normal; el default continúa legacy.
