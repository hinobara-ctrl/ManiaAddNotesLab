...
<!-- PHASE-CONTRACT:C2;kind=Deferred;behaviorChange=null;authorization=N/A -->
...
**Behavior change?:** sí si se reautoriza como A/B futuro; la fase permanece diferida y no declara un cambio actual.
...
**Estado: COMPLETE — OUTCOME READY / NOT AUTHORIZED.** El contrato `phase-d1-gate-behavioral-experiment-contract.1` quedó congelado con hash `D574631B3E713AC3D08C159B605A742D50C907B1BB4589D7FCADCFA8027A7824`. Define un único experimento futuro para `k=1 → k=2`, evidencia `ReducedOnly`, admisión sólo con joint witness exacto, abstención sin reroll, cero RNG del gate y rollback a `legacy-experimental.1`. Auditó el punto de inserción posterior a una propuesta legal y anterior a `added.Add`/`geometry.Insert`. No ejecutó A/B ni conectó evidencia a generation.
...
<!-- PHASE-CONTRACT:D1.SAFETY;kind=ResearchShadow;behaviorChange=false;authorization=RESEARCH_COMPLETED_PARKED -->

**Estado: COMPLETE — OUTCOME C / PARKED.** El oracle sintético separó raw relation, hard violation y causal attribution, con semántica específica para tap/LN/articulation/serialization, cero RNG y sin callsites productivos. Durante el forensic de outputs históricos, el prototipo atribuyó condiciones a legacy por ausencia en source sin mutation provenance; el contrato exigía `Unattributable`. Hard abort, agregado retirado y ninguna reinterpretación de D1.

**Behavior change?:** no.

**Resultado:** infraestructura parcial no autorizada para futuros gates. Exact direct/downstream/articulation/serialization attribution requiere provenance capturada durante la mutación; no puede inferirse desde snapshots finales.
...
**Artifacts:** pre-human certificate, prototype report y once summaries `e_*`; detail target/donor local e ignorado.
...
Resultado: recurrence exacta encontró 10.980 relations y reconstruyó 4.143 targets en las 11 familias, pero introdujo 1.451 mismatch y 16.224 no-context. Stable-run boundaries fueron muy estables bajo holdout, aunque cayeron 288→89 al cambiar el mínimo 2→3. Por eso recurrence y segmentation cierran B por separado; no se selecciona ningún método ni se crea Section.
...
**Estado: COMPLETE — OUTCOME B — CONTINUE CONDITIONALLY.**
...
**Resultado:** domain hashing/semantic identity y full-transition truth son auditables. Synthetic controla la lógica pero no autoriza human vocabulary. La ruta pre-export mapper-authored es viable en concepto y aún no está disponible; no se abre F2.4.

## Phase F2.ACQ — Controlled Pre-Serialization Ground Truth Acquisition Prerequisite

<!-- PHASE-CONTRACT:F2.ACQ;kind=BlockedPrerequisite;behaviorChange=false;authorization=CONDITIONAL_ON_EXTERNAL_DATA -->

**Estado: BLOCKED / CONDITIONAL ON EXTERNAL DATA / NOT ACTIONABLE.**

**Goal:** obtener packages mapper-authored autorizados con coordinates source/destination previas al export, timing map, LN head/release y `.osu`, separando DomainDesignSet de IndependentValidationSet.

**Behavior change?:** no; el prerequisite está bloqueado y no gobierna generación.

**Acceptance criteria:** recurso realmente adquirido, provenance/consent, cuatro transition kinds, timing changes, LN/rice y varios keymodes. Si no puede obtenerse, PARK F2.
...
**FUTURE / NOT_AUTHORIZED.** No existe policy, toggle, CLI/Web path ni A/B activo. SAFETY.REMEDIATION.GATE conserva cero violaciones treatment, pero su validation hardening está `NEEDS_REVIEW`; no reconsidera ni promueve G1.GATE. G1 todavía requiere revisión humana y autorización independiente.
