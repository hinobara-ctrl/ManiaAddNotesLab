...
El roadmap y sus límites están en [MAPPER_DERIVED_IMPLEMENTATION_ROADMAP.md](MAPPER_DERIVED_IMPLEMENTATION_ROADMAP.md). Phase B agrega certificados, identidades provisionales y causas por lane en shadow opt-in. `HardValidityResult` permanece binario; blockers de CurrentGeometry no se convierten en evidencia. C1 añadió únicamente reconstrucción held-out y un weight alternativo en shadow. Su Research Gate quedó **HOLD**; C1.1 separó witness identity de relation agreement; C1.2 modeló relaciones exact-head; D0 reconstruyó chord completions; D0.1 midió ocho vistas paralelas; D0.2 separó agreement marginal, donor overlap y joint witness; F1 formalizó support, mismatch, abstención y ambiguity por candidate. Generation, RNG y `BehaviorPolicyVersion=legacy-experimental.1` siguen intactos. F2 es sólo NEXT y no está autorizado; D1 tampoco está autorizado y C2 permanece deferred.
