...
El roadmap y sus límites están en [MAPPER_DERIVED_IMPLEMENTATION_ROADMAP.md](MAPPER_DERIVED_IMPLEMENTATION_ROADMAP.md). Phase B agrega certificados, identidades provisionales y causas por lane en shadow opt-in. `HardValidityResult` permanece binario; blockers de CurrentGeometry no se convierten en evidencia. C1 añadió únicamente reconstrucción held-out y el weight alternativo `DistinctObservationDistanceSumThenCandidateAffinity` en shadow. Su Research Gate quedó **HOLD**; C1.1 separó witness identity de relation agreement; C1.2 modeló relaciones exact-head; D0 reconstruyó chord completions; D0.1 midió ocho vistas paralelas y D0.2 separó agreement marginal, donor overlap y joint witness. Generation, RNG y `BehaviorPolicyVersion=legacy-experimental.1` siguen intactos. F1 es sólo el siguiente candidato shadow; D1 y C2 no están autorizados.
