...
D1.0, D1.GATE y D1 son contratos distintos. **D1.0 — Resulting-State Composition Feasibility / Shadow** cerró Outcome A sin conducta: `CompletionSetIdentity` y `ResultingHeadStateIdentity` representan composiciones canónicas, mientras `ObservedJointCompletionSet` exige una misma occurrence original elegible. El pair holdout excluye el grupo target completo y separa hard invalidity, abstención, soporte marginal y joint evidence. **D1.GATE** cerró `READY` congelando un evaluador puro y un contrato machine-readable para la transición `k=1 → k=2`: sólo admite joint evidence exacta en `ReducedOnly`, se abstiene sin reroll y consume cero RNG. Su punto futuro de inserción está después de construir una propuesta legacy legal y antes de mutar `added`/geometry; el tipo research no está referenciado por generation. **D1 — ChordCompletion Resulting-State A/B** conserva el contrato conductual futuro, versionado y con rollback exacto a `legacy-experimental.1`; continúa `NOT_AUTHORIZED`.
