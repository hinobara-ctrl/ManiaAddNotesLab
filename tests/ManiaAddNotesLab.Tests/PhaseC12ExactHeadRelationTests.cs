...
public sealed class PhaseC12ExactHeadRelationTests
{
    ...
    public void ProjectStateAndMasterStateBlocksAreConsistent()
    {
        ...
        Assert.Contains("Current phase: F1", readme);
        Assert.Contains("Current phase: F1", status);
        Assert.Contains("Next recommended phase: F2", readme);
        Assert.Contains("Next recommended phase: F2", status);
        Assert.Contains("Tests: 319 passed / 0 failed / 0 skipped", readme);
        Assert.Contains("Tests: 319 passed / 0 failed / 0 skipped", status);
    }
