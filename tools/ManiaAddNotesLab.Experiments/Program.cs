...
if (args.Length == 4 && args[0] == "safety-remediation-gate-harden-prepare")
{
    Console.WriteLine($"SAFETY.REMEDIATION.GATE hardening contract=" +
        SafetyRemediationGateHardeningRunner.Prepare(Path.GetFullPath(args[1]),
            Path.GetFullPath(args[2]), Path.GetFullPath(args[3])));
    return;
}

if (args.Length == 6 && args[0] == "safety-remediation-gate-harden-run")
{
    var outcome = SafetyRemediationGateHardeningRunner.Run(Path.GetFullPath(args[1]),
        Path.GetFullPath(args[2]), Path.GetFullPath(args[3]), Path.GetFullPath(args[4]),
        Path.GetFullPath(args[5]));
    Console.WriteLine($"SAFETY.REMEDIATION.GATE hardening outcome={outcome}");
    Environment.ExitCode = outcome == "RECERTIFICATION_PASS" ? 0 : 1;
    return;
}

if (args.Length == 2 && args[0] == "safety-remediation-gate-contract")
{
    Console.WriteLine($"SAFETY.REMEDIATION.GATE contract={SafetyRemediationGateRunner.WriteContract(Path.GetFullPath(args[1]))}");
    return;
}

if (args.Length == 4 && args[0] == "safety-remediation-gate-run")
{
    var outcome = SafetyRemediationGateRunner.Run(Path.GetFullPath(args[1]), Path.GetFullPath(args[2]),
        Path.GetFullPath(args[3]));
    Console.WriteLine($"SAFETY.REMEDIATION.GATE outcome={outcome}");
    Environment.ExitCode = outcome == "REMEDIATION_RUNTIME_CERTIFIED" ? 0 : 1;
    return;
}
...
if (args.Length == 3 && args[0] == "phase-d1-prepare")
{
    ...
    Console.WriteLine($"D1 manifest {hash}");
    return;
}

if (args.Length == 5 && args[0] == "phase-d1-ab")
{
    D1BehavioralAbRunner.Run(Path.GetFullPath(args[1]), Path.GetFullPath(args[2]),
        Path.GetFullPath(args[3]), Path.GetFullPath(args[4]));
    return;
}
...
if (args.Length == 3 && args[0] == "f2-3-synthetic")
{
    F23SyntheticRunner.Run(Path.GetFullPath(args[1]), Path.GetFullPath(args[2]));
    return;
}
...
if (args.Length == 4 && args[0] == "f2-corpus")
{
    F2CorpusRunner.Run(Path.GetFullPath(args[1]), Path.GetFullPath(args[2]),
        Path.GetFullPath(args[3]));
    return;
}

if (args.Length == 4 && args[0] == "f1-corpus")
{
    F1CorpusRunner.Run(Path.GetFullPath(args[1]), Path.GetFullPath(args[2]),
        Path.GetFullPath(args[3]));
    return;
}

if (args.Length == 4 && args[0] == "d0-2-corpus")
{
    D02CorpusRunner.Run(Path.GetFullPath(args[1]), Path.GetFullPath(args[2]),
        Path.GetFullPath(args[3]));
    return;
}
