...
| G1.DESIGN — Interior Relation Admission Contract / Shadow | ✅ **COMPLETE — READY / RECERTIFIED / BEHAVIOR NOT AUTHORIZED** | 133 memberships sobre 4.226 shapes del builder, no placements; original-only normalizado y RNG-free estructural. |
...
| SAFETY.REMEDIATION.DESIGN — Hard-Validity Remediation Design | ⏳ **FUTURE / NOT_AUTHORIZED / RESEARCH ONLY** | Diseñar una remediación del mismatch sin modificar runtime ni autorizar su implementación. |
...
SAFETY.CAUSAL cerró únicamente la investigación causal posterior a G1.GATE. No existe una fase sucesora autorizada (SAFETY.REMEDIATION.DESIGN permanece **NOT_AUTHORIZED** y sería sólo research/design). La dependencia conceptual es: SAFETY.CAUSAL → SAFETY.REMEDIATION.DESIGN → implementación/gate de remediación autorizada por separado → recertificación runtime/safety → reconsideración humana de G1.GATE → sólo entonces, con autorización separada, G1 utility. G1 utility permanece **NOT_AUTHORIZED**, al igual que G2 y H; el fallo mecánico no se corrige por tuning, reroll ni promoción automática.
